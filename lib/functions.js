...
const lyric = (query) => new Promise((resolve, reject) => {
    axios.get(`https://api.genius.com/search?q=${encodeURIComponent(query)}&access_token=SUNzjuY6v9SBZgwFFZ6lkFaWbnOGyZdNhgLBvN3G4GKg7XOIh5QjZMt3pKDM0__V`)
        .then(res => {
            if (res.data.meta.status != 200) reject(res.data.meta.status + res.data.meta.message)
            const { hits } = res.data.response
            axios.get(hits[0].result.url).then(resu => {
