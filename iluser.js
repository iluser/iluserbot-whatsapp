     ...
     /*   if (isGroupMsg && isautosticker && isMedia && isImage && !isCmd) {
        ...
        case `${prefix}resend`:
                if (quotedMsgObj) {
                    ...
                    await iluser.sendFile(from, `data:${_mimetype};base64,${mediaData.toString('base64')}`, 'file', ':)', encryptMedia.id)
                } else iluser.reply(from, `reply dong tolol yang mau di resend`, id)
                break
        ...
        case `${prefix}`: 
            ...
            break
        case `${prefix}setvn`: 
            if (!isOwner && !isPremium && !isPilot) return
            ...
            if (isQuotedAudio){
                ...
                await iluser.reply(from, `Vn dengan nama ${nmfil} berhasil disimpan!`, id)
        ...
        case prefix+'delpilot':	
            ...
            break
         case prefix+'tes1': 
            if (!isPremium) return await iluser.reply(from, `Kamu bukan user premium`, id)
...
`, message.id)
        ...
        case prefix+'github':{
            ...
            if (input.includes('/')) {
                ...
                await iluser.reply(from, text, id)
                await limitAdd(serial)
            } else {
        ...
        case prefix+'findsk':
            ...
            if (isLimit(serial)) return 
                    ...
                    try {
                        ...
                        await iluser.sendFileFromUrl(from, errorurl2, 'error.png', '💔️ Maaf, terjadi kesalahan! atau kata kunci yang dicari tidak ditemukan', id)
...
*Masa Aktif*: ${cekExp.days} hari, ${cekExp.hours} jam, ${cekExp.minutes} menit`, id)
            ...
            if (isOwner){
                ...
                for (let i = 0; i < deret.length; i++) {
                    ...
                    nomorList++
                    listPremi += `${nomorList}. @${premium.getAllPremiumUser(_premium)[i].replace(/@c.us/g, '')}\n*Name*:
...
• Name : *${groupname}* 
• Members : *${totalMem}*
• Welcome : ${welgrp ? '*ACTIVATED*' : '*DEACTIVATED*'}
• Left : ${leftgrp ? '*ACTIVATED*' : '*DEACTIVATED*'}
...
🌐️ *URL*: ${url}`
       ...
       /* case prefix+'ytmp4':
        ...
        case prefix+'resi':
            ...
            cekResi(args[1], args[2]).then((result) => iluser.reply(from, result, id))
            break
        case prefix+'kebalik':
            if(isReg(obj)) return       
            if(isLimit(serial)) return
...
• *Total Rekaman:* ${recording} ${mess.iklan}`
       ...
       /* case prefix+'':
        ...
        case prefix+'ccgen':{
            ...
            iluser.reply(message.from, `${sigo.result.card}`, message.id)
            console.log(sigo)
        }
            break 
...
case prefix+'nulis3':
        ...
        case prefix+'join':
           ...
           // if (isPremium) return iluser.reply(message.from, '\n⛔ *AKSES DI TOLAK* ⛔\n\nSilahkan hubungi creator bot untuk memasukkan bot secara manual', message.id)
            ...
            if (check.status === 200) {
                ...
                //if(!chat.isReadOnly) iluser.sendText(groupId, `Halo aku iluser_BOT, Ketik ${prefix}menu Untuk Melihat List Command Ku...`)
            } else {
        ...
        case prefix+'limit':
            ...
            for(let lmt of limidat){
                if(lmt.id === serial){
                    ...
                    iluser.reply(message.from, `Sisa limit penggunaan bot kamu: *${limitCounts}*`, message.id)
                    found = true
                }
            }
            //console.log(limit)
            ...
            if (found === false){
                ...
                iluser.reply(message.from, `Sisa limit penggunaan bot kamu: *${limitCount}*`, message.id)
            }
            break
        case prefix+'run':
        ...
        case prefix+'restlimit':
            if(isOwner){
                ...
                await sleep(3000)
                await iluser.reply(from, `Restarting limit done`)
        ...
        case prefix+'unban':
        	if (!isPilot && !isOwner) return 
                ...
                banned.splice(inz, 1)
                fs.writeFileSync('./lib/database/banned.json', JSON.stringify(banned))
                iluser.reply(message.from, 'Done unbanned target', message.id)
            break
...
break
        ...
        case prefix+'setdesk':
            ...
            break
        case prefix+'bug':
            if (args.length === 1) return iluser.reply(message.from, `Kata katanya mana tolol`, message.id)
...
Subscribe t.me/iluser_BOT for more information about this bot`)
   ...
   break
    ...
    iluser.reply(from, `*NICK EPEP GENERATOR*\n\n${amjayepep1}\n`, id)
        ...
        case `${prefix}trig`:  //case nya
        case `${prefix}triggered`:
            //iluser.reply(from, mess.wait, id)
            if (isMedia && type === 'image') {
                const mediaData = await decryptMedia(message, uaOverride)
                const imageLink = await uploadImages(mediaData, `triger.${sender.id}`)
                            await iluser.sendStickerfromUrl(from, `https://api.zeks.xyz/api/triger?apikey=apivinz&img=${imageLink}`, id)
        ...
        //SCRAP MANUAL
        case prefix+'a1': {
                    const marvel = body.slice(4)
                    //if (!marvel) return iluser.reply(from, `Contoh: ${prefix}galaxy iluserbot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://flamingtext.com/logo/Design-Alien-Glow-Animation", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type('textarea[id="1-text"]', marvel);
                            await page.click('button[type="submit"]');
                            await new Promise(resolve => setTimeout(resolve, 3000));
                            const element = await page.$(
                                'div[class="ft-result-logo-image-wrapper"] > a'
                                );
                            const text = await (await element.getProperty("href")).jsonValue();
                            const urlmp4 = ({
                                url: text
                            })
                    const responses = await fetch(text);
                    const buffer = await responses.buffer();
                    await fs.writeFile(`./media/iluser.gif`, buffer)
                    await iluser.sendFile(from,'./media/iluser.gif', id)
                            iluser.sendGiphyAsSticker(from, text)
                            browser.close();
        ...
        case prefix+'cup': {
                    ...
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}cup iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/write-text-on-the-cup-392.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'cup2': {
                    ...
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}cup2 iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/put-text-on-the-cup-387.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
            ...
            break
        case prefix+'api3': {
                    const pubg2 = body.slice(6)
                    if (!pubg2) return iluser.reply(from, `Contoh: ${prefix}api3 iluserbot`, id)
                    // iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-chu/hieu-ung-chu-kim-loai-nung-onine-341.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
            ...
            break
        case prefix+'double-neon': {
                    const logo2 = body.slice(13)
                    const logo21 = logo2.split('|')[0]
                    const logo22 = logo2.split('|')[1]
                    if (!logo2) return iluser.reply(from, `Contoh: ${prefix}double-neon iluser|bot`, id)
                    //iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-chu/tao-chu-neon-nhieu-lop-78.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
            ...
            break
        case prefix+'3dcolor': {
                    const pubg2 = body.slice(9)
                    if (!pubg2) return iluser.reply(from, `Contoh: ${prefix}3dcolor iluserbot`, id)
                    // iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-chu-phong-cach-cat-giay-3d-nhieu-mau-778.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", pubg2);
            ...
            break
        case prefix+'3d-hologram': {
                    const logo2 = body.slice(13)
                    const logo21 = logo2.split('|')[0]
                    const logo22 = logo2.split('|')[1]
                    const logo23 = logo2.split('|')[2]
                    if (!logo2) return iluser.reply(from, `Contoh: ${prefix}3d-hologram iluser|bot|keren`, id)
                    //iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/tao-logo-phong-cach-may-chieu-3d-518.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", logo21);
                            await page.type("#text-1", logo22);
                            await page.type("#text-2", logo23);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
                            const element = await page.$(
                                'div[class="thumbnail"] > img'
            ...
            break
        case prefix+'green-fire': {
                    const pubg2 = body.slice(13)
                    if (!pubg2) return iluser.reply(from, `Contoh: ${prefix}green-fire iluserbot`, id)
                    // iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-chu/tao-chu-phao-sang-xanh-140.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", pubg2);
            ...
            break
        case prefix+'autumn-leaves2': {
                    const pubg2 = body.slice(16)
                    if (!pubg2) return iluser.reply(from, `Contoh: ${prefix}autumn-leaves2 iluserbot`, id)
                    // iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-chu/tao-typography-online-chu-de-la-mua-thu-358.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", pubg2);
            ...
            break
        case prefix+'dark-greentext': {
                    const pubg2 = body.slice(16)
                    if (!pubg2) return iluser.reply(from, `Contoh: ${prefix}dark-greentext iluserbot`, id)
                    // iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-chu/hieu-ung-chu-typography-online-dark-green-typo-359.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", pubg2);
        ...
        case prefix+'cup3': {
                    const pubg2 = body.slice(6)
                    if (!pubg2) return iluser.reply(from, `Contoh: ${prefix}cup3 iluserbot`, id)
                    // iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-ve/viet-chu-coc-trai-tim-194.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", pubg2);
            ...
            break
        case prefix+'galaxy-edge': {
                    const pubg2 = body.slice(13)
                    if (!pubg2) return iluser.reply(from, `Contoh: ${prefix}galaxy-edge iluserbot`, id)
                    // iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)        
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://ephoto360.com/hieu-ung-chu/viet-chu-galaxy-goc-canh-258.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", pubg2);
            ...
            break
        case prefix+'pornhub': {
                    const marvel = body.slice(9)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}pornhub iluser|bot`, id)
        ...
        case prefix+'thunder': {
                    const marvel = body.slice(9)
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}thunder iluserbot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/thunder-text-effect-online-881.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", marvel);
        ...
        case prefix+'estext': {
                    const marvel = body.slice(8)
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}estext iluserbot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/ice-cold-text-effect-862.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", marvel);
        ...
        case prefix+'blackpink': {
                    const marvel = body.slice(11)
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}blackpink iluserbot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/create-blackpink-logo-style-online-1001.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", marvel);
        ...
        case prefix+'bordir': {
                    const marvel = body.slice(8)
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}bordir iluserbot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/fabric-text-effect-online-964.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", marvel);
        ...
        case prefix+'arttext': {
                    const marvel = body.slice(9)
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}arttext iluserbot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/break-wall-text-effect-871.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", marvel);
            ...
            try {
               if (isLimit(serial)) return 
                ...
                try {
                    ...
                    const page = await browser.newPage();
                    await page.goto('https://textpro.me/create-logo-style-marvel-studios-online-971.html', {waitUntil: 'networkidle2'}); 
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                    ...
                    await browser.close()
                    await iluser.sendFileFromUrl(message.from,  text, `inifile.jpg`, `${mess.iklann}`, message.id)
                } catch (error) {
                    await iluser.reply(message.from, "Error!", message.id)
                    await browser.close()
                }
        ...
        case prefix+'marvel2': {
                    ...
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}marvel2 iluser|bot`, id)
    ...
    case prefix+'catatan': {
            ...
            break
    case prefix+'catatan3': {
                    const test = body.slice(10)
                    if (!test) return iluser.reply(from, `Contoh: ${prefix}catatan3 iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)  
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)      
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://thiepmung.com/thong-diep-yeu-thuong/card/tao-quote-anh-an-tuong-348.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", test);
            ...
            break
    case prefix+'catatan4': {
                    const test = body.slice(10)
                    if (!test) return iluser.reply(from, `Contoh: ${prefix}catatan4 iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)  
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)      
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://thiepmung.com/thong-diep-yeu-thuong/card/viet-thong-dien-len-giay-kep-340.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", test);
            ...
            break
    case prefix+'hearttext': {
                    const test = body.slice(11)
                    if (!test) return iluser.reply(from, `Contoh: ${prefix}hearttext iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)  
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)      
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://thiepmung.com/tao-thiep-tinh-yeu/card/viet-ten-len-trai-tim-nghe-thuat-396.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", test);
            ...
            try {
               if (isLimit(serial)) return 
                ...
                try {
                    ...
                    const page = await browser.newPage();
                    await page.goto('https://thiepmung.com/hieu-ung-chu-anh/name-heart/viet-ten-len-hinh-trai-tim-194.html', {waitUntil: 'networkidle2'});  
                    await page.click("#radio0-radio-1bf9db5be8bb486eb8193c774726e5ba");
                    await page.type("#text-0", tes5);
                    await page.keyboard.press('Enter');
                    //await page.waitForSelector('div[class="thumbnail"] img', {visible: true})
                    await new Promise(resolve => setTimeout(resolve, 5000));
                    const element = await page.$(
                                'div[class="thumbnail"] > img'
                                );
                ...
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            ...
                            iluser.sendFileFromUrl(from, text, 'tes.jpg', `${mess.iklann}`, id)
                            browser.close();
            ...
            try {
               if (isLimit(serial)) return 
                ...
                try {
                    ...
                    const page = await browser.newPage();
                    await page.goto("https://ephoto360.com/tao-hinh-nen-12-cung-hoang-dao-full-hd-cho-mobile-710.html", {waitUntil: 'networkidle2'});  
                    ...
                    await new Promise(resolve => setTimeout(resolve, 5000));
                    //await page.waitForSelector('div[class="success"]', {visible: true})
                    let spanHref = await page.$eval('div[class="thumbnail"] > img', span => span.getAttribute('src'));  
                    await browser.close()
                    await iluser.sendFileFromUrl(message.from,  spanHref, `inifile.jpg`, `${mess.iklann}`, message.id)
                } catch (error) {
                    await iluser.reply(message.from, "Error!", message.id)
                    await browser.close()
                }
                ...
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            ...
                            const responses = await fetch(text);
                            const buffer = await responses.buffer();
                            await fs.writeFile(`./media/iluser.jpg`, buffer)
                            await iluser.sendFile(from,'./media/iluser.jpg', 'iluser.jpg', `${mess.iklann}`, id)
                            browser.close();
    ...
    case prefix+'api2': {
                    const tes15 = body.slice(6)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}api2 iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/realistic-flaming-text-effect-online-197.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
    ...
    case prefix+'harrypotter': {
                    ...
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}harrypotter iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/create-harry-potter-text-on-horror-background-178.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
    ...
    case prefix+'mememaker': {
        ...
        case prefix+'luv2': {
            if(isReg(obj)) return
                    ...
                    const tes151 = tes15.split('|')[0]
                    const tes152 = tes15.split('|')[1]
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}luv2 iluser | bot`, id)
                    // iluser.reply(from, mess.wait, id)       
            if (isLimit(serial)) return 
            await limitAdd(serial)
                    const puppeteer = require('puppeteer')
        ...
        case prefix+'neon2': {
                    const tes15 = body.slice(7)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}neon2 iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
        ...
        case prefix+'smoke': {
                    const tes15 = body.slice(7)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}smoke iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/other-design/create-an-easy-smoke-type-effect-390.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'kertas': {
                    ...
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}kertas iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/shadow-text-effect-in-the-sky-394.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'pahat': {
                    const tes15 = body.slice(7)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}pahat iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/carved-wood-effect-online-171.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'kopi': {
                    const tes15 = body.slice(6)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}kopi iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/put-your-text-on-a-coffee-cup--174.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'kertas2': {
                    const tes15 = body.slice(9)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}kertas2 iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/write-text-on-burn-paper-388.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'luv3': {
                    const tes15 = body.slice(6)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}luv3 iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/write-art-quote-on-wood-heart-370.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
            ...
            break
        case prefix+'autumn-leaves': {
                    const tes15 = body.slice(15)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}autumn-leaves iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/quotes-under-fall-leaves-347.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
            ...
            break
        case prefix+'neoncolor': {
                    const tes15 = body.slice(11)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}neoncolor iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/make-smoky-neon-glow-effect-343.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_0", tes15);
                            await page.click("#login");
                            await new Promise(resolve => setTimeout(resolve, 3000));
                            const element = await page.$(
                                'div[class="thumbnail"] > img' 
        ...
        case prefix+'googletext': {
                    ...
                    const tes151 = tes15.split('|')[0]
                    const tes152 = tes15.split('|')[1]
                    const tes153 = tes15.split('|')[2]
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}googletext iluser_BOT | iluser_BOT made by iluser | iluser_BOT paling debes`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/other-design/make-google-suggestion-photos-238.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes151);
                            await page.type("#text_2", tes152);
                            await page.type("#text_3", tes153);
                            await page.click("#login");
                            await new Promise(resolve => setTimeout(resolve, 3000));
                            const element = await page.$(
                                'div[class="thumbnail"] > img' 
        ...
        case prefix+'langit': {
                    const tes15 = body.slice(8)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}langit iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/write-stars-text-on-the-night-sky-200.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'matrix': {
                    const tes15 = body.slice(8)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}matrix iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/text-under-web-matrix-effect-185.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.click("#optionNumber_0-5-Form");
                            await page.type("#text_2", tes15);
                            await page.click("#login");
                            await new Promise(resolve => setTimeout(resolve, 3000));
                            const element = await page.$(
                                'div[class="thumbnail"] > img' 
            ...
            break
        case prefix+'lamptext': {
                    const tes15 = body.slice(10)
                    if (!tes15) return iluser.reply(from, `Contoh: ${prefix}lamptext iluser_BOT`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://photooxy.com/logo-and-text-effects/illuminated-metallic-effect-177.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text_1", tes15);
        ...
        case prefix+'chalktext': {
            ...
            break
        case prefix+'crowntext': {
                    const marvel = body.slice(11)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/create-text-logo-3d-metal-online-957.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.click("#radio0-radio-b76c3d367c53460d84ed1d1b45b9b8ce");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext1': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext1 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-7a2d868398eb43e4bed7b71c5f3932d6");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext2': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext2 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-96b78c51c4604575b8459e397b3ee8aa");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext3': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext3 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-0c1b4df44e9a43039f1f69af6bc4f769");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext4': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext4 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-f035036226514359a7775ce0771dd2ae");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext5': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext5 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-3aa9fc3e192a4d7c8f7c5e03ba0713f9");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext6': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext6 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-2f22ac70bcf448b0932cd6d7f6e113fe");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext7': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext7 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-020033e58ab74b06b27d86cfe5f273db");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext8': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext8 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-dac1df8754c44c14a797756c6e61ab90");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext9': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext9 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-44428dfc3850425c8186442b36d21b71");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
            ...
            break
        case prefix+'crowntext0': {
                    const marvel = body.slice(12)
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}crowntext0 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    await page
                        ...
                        .then(async () => {
                            await page.click("#radio0-radio-d26b762b49324ddaaba3c7a9793531e8");
                            await page.type("#text-0", marvel1);
                            await page.type("#text-1", marvel2);
                            await page.click("#submit");
                            await new Promise(resolve => setTimeout(resolve, 3000));
        ...
        case prefix+'blood2': {
            ...
            break
        case prefix+'blood3': {
                    const tes1 = body.slice(8)
                    if (!tes1) return iluser.reply(from, `Contoh: ${prefix}blood3 iluser bot`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/blood-text-on-the-frosted-glass-941.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            //await page.click("#radio0-radio-a53b07c2e8354a16b5a14e34bfbe666f");
        ...
        case prefix+'halloween': {
                    const tes1 = body.slice(11)
                    if (!tes1) return iluser.reply(from, `Contoh: ${prefix}halloween iluser bot`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/halloween-fire-text-effect-940.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            //await page.click("#radio0-radio-a53b07c2e8354a16b5a14e34bfbe666f");
            ...
            break
        case prefix+'joker': {
                    const tes1 = body.slice(7)
                    if (!tes1) return iluser.reply(from, `Contoh: ${prefix}joker iluser bot`, id)
                    // iluser.reply(from, mess.wait, id)       
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            await limitAdd(serial)
        ...
        case prefix+'3dtext': {
                    const marvel = body.slice(8)
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}3dtext iluser_BOT`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/3d-box-text-effect-online-880.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", marvel);
        ...
        case prefix+'3dtext2': {
                    ...
                    const marvel1 = marvel.split('|')[0]
                    const marvel2 = marvel.split('|')[1]
                    if (!marvel) return iluser.reply(from, `Contoh: ${prefix}3dtext2 iluser|bot`, id)
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://textpro.me/3d-steel-text-effect-877.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", marvel1);
        ...
        case prefix+'undangan': {
                    const logo2 = body.slice(10)
                    const logo21 = logo2.split('|')[0]
                    const logo22 = logo2.split('|')[1]
                    const logo23 = logo2.split('|')[2]
                    const logo24 = logo2.split('|')[3]
                    //if (!logo2) return iluser.reply(from, `Contoh: ${prefix}double-neon iluser|bot`, id)
                    //iluser.reply(from, mess.wait, id)
            if(isReg(obj)) return
            if (isLimit(serial)) return 
            ...
            try {
                (async () => {
                    ...
                    const page = await browser.newPage();
                    await page
                        .goto("https://birthdaycake24.com/elegant-wedding-invitation-card-with-flowers-1693.html", {
                            waitUntil: "networkidle2"
                        })
                        .then(async () => {
                            await page.type("#text-0", logo21);
